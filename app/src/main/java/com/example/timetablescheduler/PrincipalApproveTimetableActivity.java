...
public class PrincipalApproveTimetableActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_principal_approve_timetable);
    ...
    private void loadGeneratedTimetables() {
        ...
        query.whereEqualTo("isApprovedByDean", true);
        ...
        query.findInBackground((timetables, e) -> {
            if (e == null && timetables != null && !timetables.isEmpty()) {
                ...
                for (ParseObject timetable : timetables) {
                    ...
                    batchNames.add(name + (section.isEmpty() ? "" : " - " + section)
                            + (year.isEmpty() ? "" : " (" + year + ")"));
                }
                ...
                batchSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                    @Override public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
                        ...
                        isApproved = timetable.has("isApprovedByPrincipal") && timetable.getBoolean("isApprovedByPrincipal");
                ...
                showMessage("No timetables for Principal approval.");
                clearTableAndInfo();
    ...
    private void fetchAndShowTimetable() {
        ...
        configQuery.orderByDescending("createdAt").setLimit(1);
        configQuery.include("breaks").include("periods");
        configQuery.getFirstInBackground((config, configErr) -> {
            ...
            if (workingDays == null || workingDays.isEmpty() || periods == null || periods.isEmpty()) {
                showMessage("No timetable structure configured.");
                return;
            }
    ...
    private void toggleApproval() {
        if (selectedTimetable != null) {
            ...
            selectedTimetable.put("isApprovedByPrincipal", newApproval);
            selectedTimetable.saveInBackground(e -> {
                if (e == null) {
                    ...
                    Toast.makeText(this, isApproved ?
                            "Timetable approved by Principal!" : "Principal approval removed!", Toast.LENGTH_SHORT).show();
                } else {
    ...
    private void renderTimetable(TableLayout tableLayout, List<String> workingDays, List<ParseObject> periods,
                                 List<ParseObject> breaks, String[][][] timetableData) {
        tableLayout.removeAllViews();
        infoLayout.removeAllViews();
